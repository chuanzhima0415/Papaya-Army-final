...
struct StorageManager {
	struct UserDefault {
		...
		func saveDataToUserDefault(_ data: Codable, forKey key: String) async {
			do {
		...
		func loadDataFromUserDefault<T: Codable>(forKey key: String) async -> T? {
	...
	struct FileManagers<T: Codable> {
		...
		func saveDataToFileManager(_ data: T?) async {
			do {
				let encoder = JSONEncoder()
				let dataInJson = try encoder.encode(data)
		...
		func loadDataFromFileManager() async -> T? {
			do {
