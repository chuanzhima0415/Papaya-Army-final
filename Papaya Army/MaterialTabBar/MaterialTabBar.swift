        ...
        .background {
            Group {
                if selection == tab {
                    ...
                    // 当 selection == .standing 时，此时整个屏幕只有 standing tab 才有 matchedGeometryEffect, 因此 matchedGeometryEffect 不用指定 isSource 是否为 true
                }
            }
        }
        .contentShape(contentShape)  // 使点击区域的形状为 contentShape(原来是 Image + Text 的 VStack 矩形)
        .onTapGesture { // 用户点击此 tabView (此时该 tabView 并没有被选中)
            switchTo(newTab: tab)
        }
    }
    
    /// 切换到点击后选的 tab 按钮
    /// - Parameter newTab: 新选中的 tab
    private func switchTo(newTab: TabModel) {
